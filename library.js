...
const writingPrivileges = ["topics:create", "topics:reply"];
...
exports.load = function ({ app, middleware, router }, next) {
										...
										async.waterfall(
											[
												...
												function (categoriesData, next) {
													...
													categoriesData = Categories.buildForSelectCategories(
														categoriesData
													);
						...
						function (data) {
							...
							Privileges.userPrivilegeList
								...
								.forEach((priv, i) => {
									...
									if (!settings.get(priv)) {
										data.privileges.labels.users.splice(
											Privileges.userPrivilegeList.indexOf(priv),
											1
										);
							...
							Privileges.groupPrivilegeList
								...
								.forEach((priv, i) => {
									...
									if (!settings.get(priv)) {
										data.privileges.labels.groups.splice(
											Privileges.groupPrivilegeList.indexOf(priv),
											1
										);
							...
							data.privileges.labels.users.splice(
								data.privileges.labels.users.indexOf("assigngroups"),
								1
							);
							Object.keys(data.privileges.users).forEach((uid) => {
							...
							data.privileges.labels.groups.splice(
								data.privileges.labels.groups.indexOf("assigngroups"),
								1
							);

							Object.keys(data.privileges.groups).forEach((group) => {
								delete data.privileges.groups[group].privileges[
									"groups:assigngroups"
								];
							});
							...
							res.render("modmin/category", {
								...
								isGlobal: cid == 0 ? "true" : "",
								forceOwner:
									!settings.get("force-owner") || isGroupAssigner || cid != 0
										? ""
										: "true",
							});
						},
	...
	async function uploadCategoryPicture(req, res, next) {
		try {
			...
			if (isUserAllowedTo) {
				return await controllers.admin.uploads.uploadCategoryPicture(
					req,
					res,
					next
				);
	...
	router.post("/api/modmin/uploadpicture", middlewares, uploadCategoryPicture);

	SocketPlugins.modmin = { categories: {} };

	SocketPlugins.modmin.categories.getPrivilegeSettings = socketMethod(
		(socket, { cid }, callback) => {
			async.waterfall(
				[
					(next) => {
					...
					(privileges, next) => {
						Privileges.userPrivilegeList
							...
							.forEach((priv, i) => {
								if (priv === "moderate") return;
								if (!settings.get(priv)) {
									privileges.labels.users.splice(
										Privileges.userPrivilegeList.indexOf(priv),
										1
									);
						...
						// Assign groups is admin only.
						privileges.labels.users.splice(
							privileges.labels.users.indexOf("assigngroups"),
							1
						);
						Object.keys(privileges.users).forEach((uid) => {
	...
	SocketPlugins.modmin.categories.setPrivilege = socketMethod(
		(socket, data, callback) => {
			...
			if (
				!(
					parseInt(data.member, 10) &&
					parseInt(data.member, 10) + "" === data.member + ""
				) &&
				!settings.get("manage-groups")
			)
			...
			if (Array.isArray(data.privilege)) {
					...
					function (privilege, next) {
						...
						if (privilege === "modmin" && uid === parseInt(data.member, 10)) {
							if (data.privilege.length === 1) {
								return callback(
									new Error(`Can't revoke your own Manage privilege.`)
								);
							} else {
								return next();
							}
						...
						Groups[data.set ? "join" : "leave"](
							"cid:" + data.cid + ":privileges:" + privilege,
							data.member,
							next
						);
					},
				...
				if (
					!(
						settings.get(data.privilege) ||
						data.privilege === "modmin" ||
						data.privilege === "moderate"
					)
				)
					return callback(new Error("[[error:not-authorized]]"));
				...
				Groups[data.set ? "join" : "leave"](
					"cid:" + data.cid + ":privileges:" + data.privilege,
					data.member,
					onSetComplete
				);
			}
	...
	SocketPlugins.modmin.categories.copyPrivilegesToChildren = socketMethod(
		(socket, data, callback) => {
			const { uid } = socket;
			const { cid } = data;

			if (!settings.get("manage-groups"))
				return callback(new Error("[[error:not-authorized]]"));
			...
			function copyPrivilegesToChildrenRecursive(
				parentCid,
				category,
				callback
			) {
	...
	SocketPlugins.modmin.categories.copyPrivilegesFrom = (
		socket,
		{ toCid, fromCid },
		callback
	) => {
		...
		async.every(
			[fromCid, toCid],
			(cid, next) => isAdminOrModmin(cid, uid, next),
			(err, isAdminOrModmin) => {
				if (err || !isAdminOrModmin)
					return callback(new Error("[[error:not-authorized]]"));
	...
	SocketPlugins.modmin.categories.addSubcategory = socketMethod(
		(socket, data, callback) => {
			const cid = data.cid;
			const uid = socket.uid;
			let category = {};
						...
						if (!owner) {
							async.parallel(
								[
									async.apply(
										Groups.join,
										"cid:" + category.cid + ":privileges:modmin",
										uid
									),
									async.apply(
										Groups.join,
										"cid:" + category.cid + ":privileges:moderate",
										uid
									),
									async.apply(
										Groups.join,
										"cid:" + category.cid + ":privileges:deletecategories",
										uid
									),
								],
								(err) => next(err)
								...
								async.parallel(
									[
										...
										async.apply(
											...
											defaultPrivileges,
											[category.cid],
											["registered-users", "guests", "spiders"]
										),
										async.apply(
											Database.setObjectField,
											"modmin:cid:group",
											`${category.cid}`,
											group
										),
	...
	SocketPlugins.modmin.categories.editCategory = socketMethod(
		(socket, data, callback) => {
			const cid = data.cid;
	...
	SocketPlugins.modmin.categories.addGroup = socketMethod(
		(socket, data, callback) => {
			const { cid } = data;
			const { uid } = socket;
	...
	SocketPlugins.modmin.categories.deleteCategory = socketMethod(
		(socket, data, callback) => {
			const { cid } = data;
			const { uid, ip } = socket;
				...
				if (!settings.get("delete-or-disable")) {
					async.waterfall(
						[
							...
							async () => {
								return await Groups.leave(
									"cid:" + cid + ":privileges:modmin",
									uid
								);
							},
						],
	...
	SocketPlugins.modmin.categories.addCategory = socketMethod(
		(socket, data, callback) => {
			const uid = socket.uid;
			let category = {};
			let isAdmin = false;
			let usernames = [];
			let fields = data.data;
			let owner = !!data.owner ? data.owner : socket.uid;
			let userTitleEnabled = data.userTitleEnabled ? 1 : 0;
			let group = data.group ? 1 : 0;
			...
			async.waterfall(
				[
					...
					(next) => {
						User.getUsernamesByUids([uid, owner], (err, result) => {
							if (!err) {
								usernames = result;
								next();
							}
					...
					(next) => {
						if (!isAdmin && settings.get("disable-on-creation")) {
							notifications.create(
								{
									type: "post-queue",
									bodyShort: `[[modmin:new_category, '', ${category.name}, ${
										category.cid
									}, ${usernames[0]}, ${uid}]]${
										owner != uid
											? `. [[modmin:new_category_owner, '', ${usernames[1]}, ${owner}]]`
											: ""
									}`,
									nid: "new_category:" + category.name,
									path: `/admin/manage/categories/${category.cid}`,
							...
							async.parallel(
								[
									...
									async.apply(
										...
										writingPrivileges,
										[category.cid],
										!!settings.get("rescind-defaults-writing")
											? ["registered-users", "guests", "spiders"]
											: []
									),
									async.apply(
				...
				(err) =>
					callback(err, {
						cid: category.cid,
						disabled: settings.get("disable-on-creation"),
					})
			);
		}
	);
...
function isAdminOrGroupAssigner(cid, uid, callback) {
	async.parallel(
		...
		(err, results) =>
			callback(err, err ? false : results.isAdmin || results.isGroupAssigner)
	);
}
	...
	async.parallel(
		{
			...
			canDelete(next) {
				Helpers.isUserAllowedTo(
					...
					uid,
					[cid],
					(err, isAllowed) => next(err, isAllowed ? isAllowed[0] : false)
				);
			},
		},
		(err, results) =>
			callback(err, err ? false : results.isAdmin || results.canDelete)
	);
}
